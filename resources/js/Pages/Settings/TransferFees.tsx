...
import { TransferFee, TransferFeePageProps } from '@/types';

export default function TransferFees({ tiers }: TransferFeePageProps) {
  ...
  function destroy(tier: TransferFee) {
    if (confirm('Remove this fee tier?')) {
      router.delete(`/settings/fee-tiers/${tier.id}`, { preserveScroll: true });
    }
  }

  return (
    <AppLayout>
      <Head title="Cash in / Cash out Fee" />

      <div className="mb-6">
        <h1 className="font-display text-2xl font-bold md:text-3xl">Cash in / Cash out Fee</h1>
        <p className="text-sm text-clay-textSoft">
          Configure how much profit (fee) is earned per transaction amount range. New transactions use these tiers automatically.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="clay overflow-hidden p-2">
            <table className="w-full border-separate border-spacing-y-1 text-sm">
              ...
              <tbody>
                {tiers.map((tier) => (
                  <tr key={tier.id} className="clay-sm">
                    <td className="px-4 py-3 font-semibold">
                      ₱{tier.min_amount.toLocaleString()} {tier.max_amount ? `– ₱${tier.max_amount.toLocaleString()}` : 'and up'}
                    </td>
                    <td className="px-4 py-3">
                      <span className="clay-chip bg-clay-successSoft px-3 py-1 text-xs font-bold text-clay-success">
                        ₱{parseFloat(tier.fee).toFixed(2)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => destroy(tier)} className="clay-sm px-3 py-1.5 text-xs font-bold text-clay-danger">
                        Delete
                      </button>
                    </td>
                  </tr>
                ...
                {tiers.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-10 text-center text-clay-textFaint">
                      No fee tiers configured yet — add one to start calculating fees.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div>
          <form onSubmit={submit} className="clay flex flex-col gap-4 p-6">
            <h3 className="font-display text-lg font-bold">Add a tier</h3>
